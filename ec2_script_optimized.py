...
from Servicios.cloudtrail_functions_optimized import get_ec2_cloudtrail_events, insert_or_update_cloudtrail_events
...
def process_account_region(account_id, role_name, account_name, region, requested_services):
    ...
    try:
        # Establecer un tiempo límite para el procesamiento de cada cuenta/región
        timeout = 300  # 5 minutos máximo por cuenta/región
        start = datetime.now()
        
        for service in requested_services:
            # Verificar si hemos excedido el tiempo límite
            if (datetime.now() - start).total_seconds() > timeout:
                print(f"[{account_id}:{region}] Tiempo límite excedido, omitiendo servicios restantes")
                break
            ...
            try:
                ...
                print(f"[{account_id}:{region}] Error en servicio {service}: {str(e)}")
                # Continuar con el siguiente servicio en lugar de fallar todo el proceso
...
def main(requested_services):
    ...
    print(f"Usando {max_workers} workers para procesamiento paralelo")
    
    collection_start = datetime.now()
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = []
        
        # Procesar solo un subconjunto de cuentas para pruebas
        test_mode = True
        if test_mode:
            test_accounts = ROLES[:5]  # Usar solo las primeras 5 cuentas
            print(f"MODO PRUEBA: Procesando solo {len(test_accounts)} cuentas")
            for role in test_accounts:
